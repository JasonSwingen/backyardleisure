...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	$accountLog		= get_page_by_title($OPTION['wps_pgNavi_logOption']);	//change.9.1
	...
	if($login_check == 1){
		...
		if($customerArea == NULL){
			echo "<p class='error'>".$ERROR_DETAILS['0007']."</p>"; exit();
		}
		if($accountReg == NULL){
			echo "<p class='error'>".$ERROR_DETAILS['0008']."</p>"; exit();
		}
		if($accountLog == NULL){
			echo "<p class='error'>".$ERROR_DETAILS['0009']."</p>"; exit();
		}
